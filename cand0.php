...
if($_GET['page'] == "login"){
    ...
    $db = dbconnect();
    if(preg_match('/prob|_|\.|\(\)/i', $input['id'])) exit("No Hack ~_~");
...
if($_GET['page'] == "upload"){
    ...
    if(preg_match('/prob|_|\.|\(\)/i', $_FIELS['fileToUpload'])) exit("No Hack ~_~");
    ...
    if($extension == "txt" || $extension == "png"){
        system( "cp {$_FILES['fileToUpload']['tmp_name']} ./upload/{$_FILES['fileToUpload']['name']}" ) !== 0 or die('Extract failed.');
...
if($_GET['page'] == "download"){
    ...
    $fil_file = (isset($_REQUEST['file']) && preg_match("/[a-z0-9]{32}/",$_REQUEST['file'])) ? $_REQUEST['file'] : false;
    $fil_file = basename($_GET["file"]);
    $content = file_get_contents("./upload/{$fil_file}");
